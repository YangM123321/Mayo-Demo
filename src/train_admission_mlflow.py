# src/train_admission_mlflow.py  (header/config section)
...
# 🔧 Nuke any env overrides that might confuse MLflow on Windows
os.environ.pop("MLFLOW_TRACKING_URI", None)
os.environ.pop("MLFLOW_REGISTRY_URI", None)

# ✅ Use SQLite for BOTH tracking & registry (bulletproof on Windows)
...
# --- Load data & build label (same as training script) ---
...
# --- Tiny-data safe split (avoid stratify crash) ---
counts = Counter(y)
...
with mlflow.start_run(run_name="logreg_baseline"):
    ...
    # best-effort AUC (works even when no split; identical sets)
    ...
    # optional: text report to artifacts
    try:
        report = classification_report(yte, model.predict(Xte), zero_division=0)
        (OUT / "ml_report.txt").write_text(report)
        mlflow.log_artifact(str(OUT / "ml_report.txt"))
    except Exception:
        pass

    # log the model and also save a copy for your API
